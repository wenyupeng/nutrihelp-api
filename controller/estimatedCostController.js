let {getRecipeIngredients} = require('../model/recipesModel');
let {getIngredientsPriceById} = require('../model/IngredientPriceModel');
...
const getFullCost = async (req, res) => {
  ...
  try {
    // Get recipe data
    const data = await getRecipeIngredients(recipe_id);
    if (data.length === 0) {
    ...
    const ingredients_price = await getIngredientsPriceById(ingredients.id);
